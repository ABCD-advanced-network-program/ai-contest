    ...
    def get(self, request):
        if request.user.is_staff:
            try:
                ...
                userName = ''
            obj = models.Contest.objects.filter(
                IDUser=request.user.id).order_by('-id')
            # import datetime
...
class ContestDetail(View):
    def get(self, request, id):
        if request.user.is_staff:
            ...
            data = []
            obj = models.RegisterContest.objects.filter(
                IDContest=id).order_by('-id')
            for x in obj:
                tg = {
    ...
    def post(self, request, id):
        ...
        obj.TimeRegister = request.POST.get(
            'dateRegister')+' '+request.POST.get('timeRegister')
        obj.TimeStart = request.POST.get(
            'dateStart')+' '+request.POST.get('timeStart')
        obj.TimeEnd = request.POST.get(
            'dateEnd')+' '+request.POST.get('timeEnd')
...
class CreateContest(View):
    ...
    def post(self, request):
        try:
            ...
            messages.error(
                request, 'Create contest is not successful, please fill in full fields')
